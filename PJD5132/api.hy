...
(import serial [PJD5132.commands [reads? writes? describe octet octet/big]])
...
(defmacro $ [name stat &rest methods]
  ...
  (import itertools [PJD5132.commands [*command-map* *command-types*]])
  ...
  (defn make-method [name op]
    ...
    (setv type (.get *command-types* op 'nil))
    (if (= type 'nil) (setv type `do))

    (if (is operator nil) `(do)
      `(defn ~name [self] (~type (self.send-command [~@(.get *command-map* op)])))))
